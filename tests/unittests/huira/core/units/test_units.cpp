...
TEST_CASE("Basic unit construction and value retrieval", "[construction]") {
    SECTION("Length units") {
        ...
        REQUIRE(m.to_si() == 100.0);

        Kilometer km{ 1 };
        REQUIRE(km.value() == 1.0);
        REQUIRE(km.to_si() == 1000.0);

        Millimeter mm{ 1000 };
        REQUIRE(mm.value() == 1000.0);
        REQUIRE(mm.to_si() == 1.0);
    }

    SECTION("Angle units") {
        Radian rad{ 1.0 };
        REQUIRE(rad.value() == 1.0);
        REQUIRE(rad.to_si() == 1.0);

        Degree deg{ 180 };
        REQUIRE(deg.value() == 180.0);
        REQUIRE(std::abs(deg.to_si() - PI<double>()) < EPSILON);
    }

    SECTION("Time units") {
        Second s{ 60 };
        REQUIRE(s.value() == 60.0);
        REQUIRE(s.to_si() == 60.0);

        Minute min{ 1 };
        REQUIRE(min.value() == 1.0);
        REQUIRE(min.to_si() == 60.0);

        Hour h{ 1 };
        REQUIRE(h.value() == 1.0);
        REQUIRE(h.to_si() == 3600.0);
    }
}
...
TEST_CASE("Mixed-scale arithmetic preserves left-hand scale", "[arithmetic][mixed-scale]") {
    SECTION("Addition preserves LHS scale") {
        ...
        REQUIRE(std::abs(c.to_si() - PI<double>() / 2) < EPSILON);
    }
...
TEST_CASE("Quantity multiplication creates composite units", "[arithmetic][multiplication]") {
    ...
    SECTION("Power * Time = Energy") {
        ...
        REQUIRE(energy.to_si() == 1000.0);  // Joules
    }
    ...
    SECTION("Mixed scale multiplication") {
        ...
        REQUIRE(std::abs(speed.to_si() - (5000.0 / 7200.0)) < EPSILON);
    }
}
...
TEST_CASE("User-defined literals", "[literals]") {
    ...
    SECTION("Time literals") {
        ...
        REQUIRE(h.to_si() == 7200.0);
        REQUIRE(min.to_si() == 1800.0);
        REQUIRE(s.to_si() == 45.0);
        REQUIRE(ms.to_si() == 0.5);
    }
    ...
    SECTION("Power literals") {
        ...
        REQUIRE(kw.to_si() == 1500.0);
        REQUIRE(mw.to_si() == 1000.0);
    }
    ...
    SECTION("Literals in expressions") {
        ...
        REQUIRE(speed.to_si() == 10.0);  // 10 m/s
    }
}
...
TEST_CASE("Complex unit compositions", "[composite]") {
    SECTION("Radiance: W / (m^2  sr)") {
        ...
        REQUIRE(std::abs(radiance.to_si() - 150.0) < EPSILON);
    }
    ...
    SECTION("Irradiance: W / m^2") {
        ...
        REQUIRE(std::abs(irradiance.to_si() - 10.0) < EPSILON);
    }
    ...
    SECTION("Kinetic energy: 1/2 m v^2") {
        ...
        REQUIRE(std::abs(energy.to_si() - 500.0) < EPSILON);
    }
    ...
    SECTION("Angular velocity") {
        ...
        // In radians: 2pi / 60s
        REQUIRE(std::abs(angular_velocity.to_si() - (2 * PI<double>() / 60.0)) < EPSILON);
    }
}
...
TEST_CASE("Real-world rendering scenarios", "[rendering]") {
    SECTION("FOV conversions") {
        ...
        double fov_radians = user_fov.to_si();
    ...
    SECTION("Light intensity calculations") {
        ...
        REQUIRE(std::abs(irradiance.to_si() - 0.318) < 0.001);
    }
}
...
TEST_CASE("Copy and assignment semantics", "[semantics]") {
    SECTION("Copy construction") {
        ...
        REQUIRE(copy.to_si() == original.to_si());
    }
