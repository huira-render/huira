...
int fetch_tycho2(
    const fs::path& output_dir, const Context& ctx, bool force, bool process, bool clean)
{
    huira::make_path(output_dir);

    constexpr std::string_view base_url = "https://cdsarc.cds.unistra.fr/viz-bin/nph-Cat/txt?I/259";
...
static int run_fetch(const Context& ctx, int argc, char** argv)
{
    try {
        ...
        TCLAP::SwitchArg force_arg(
            "f", "force", "Overwrite existing files instead of skipping", cmd, false);

        TCLAP::SwitchArg process_arg(
            "p", "process", "Automatically process the files once downloaded", cmd, false);

        TCLAP::SwitchArg clean_arg(
            "c", "clean", "Remove downloaded .dat files after processing", cmd, false);

        TCLAP::UnlabeledValueArg<std::string> output_arg(
            "output", "Output directory for downloaded files", true, "", "output_directory", cmd);
        ...
        return fetch_tycho2(
            output_dir, ctx, force_arg.getValue(), process_arg.getValue(), clean_arg.getValue());
...
const bool registered = [] {
    Registry::instance().add(
        {"fetch-tycho2", "Download Tycho-2 catalog from CDS Strasbourg", run_fetch});
    return true;
}();
