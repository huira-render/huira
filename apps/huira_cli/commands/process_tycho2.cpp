...
int process_tycho2(const fs::path& input_dir,
                   ...
                   bool clean)
{
    ...
    if (ctx.verbose) {
        ...
        finish_bar(bar,
                   std::to_string(all_stars.size()) + " stars written to " + output_path.string());
    }
...
static int run_process(const Context& ctx, int argc, char** argv)
{
    try {
        ...
        TCLAP::UnlabeledValueArg<std::string> input_arg(
            "input", "Input directory with tyc2.dat files", true, "", "input_directory", cmd);

        TCLAP::ValueArg<std::string> output_arg(
            "o", "output", "Output directory for tycho2.hrsc", false, "", "output_directory", cmd);
...
const bool registered = [] {
    Registry::instance().add({"process-tycho2",
