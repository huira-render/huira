    ...
    std::call_once(lsk_init_flag, []() {
        ...
        if (failed_c()) {
            ...
            if (failed_c()) {
                ...
                HUIRA_THROW_ERROR("ensure_lsk_loaded() - Failed to load default LSK: " +
                                  std::string(msg));
            }
        }
...
inline double unitim(double epoch, const std::string& insys, const std::string& outsys)
{
    ...
    return static_cast<double>(
        call_spice(unitim_c, static_cast<SpiceDouble>(epoch), insys.c_str(), outsys.c_str()));
}
...
inline std::tuple<Vec3<T>, Vec3<T>, double> spkezr(const std::string& TARGET,
                                                   ...
                                                   const std::string& OBSERVER)
{
    ...
    Vec3<T> position_km{
        static_cast<T>(state[0]), static_cast<T>(state[1]), static_cast<T>(state[2])};
    Vec3<T> velocity_km{
        static_cast<T>(state[3]), static_cast<T>(state[4]), static_cast<T>(state[5])};
 ...
 */
template <huira::IsFloatingPoint T>
inline huira::Rotation<T>
pxform(const std::string& FROM, const std::string& TO, const huira::Time& time)
{
    SpiceDouble et = time.et();
    ...
    huira::Mat3<T> rotation{static_cast<T>(matrix[0][0]),
                            static_cast<T>(matrix[1][0]),
                            static_cast<T>(matrix[2][0]),
                            static_cast<T>(matrix[0][1]),
                            static_cast<T>(matrix[1][1]),
                            static_cast<T>(matrix[2][1]),
                            static_cast<T>(matrix[0][2]),
                            static_cast<T>(matrix[1][2]),
                            static_cast<T>(matrix[2][2])};
 ...
 */
template <huira::IsFloatingPoint T>
inline std::pair<huira::Rotation<T>, huira::Vec3<T>>
sxform(const std::string& FROM, const std::string& TO, const huira::Time& time)
{
    SpiceDouble et = time.et();
    ...
    huira::Vec3<T> ang_vel_local{static_cast<T>(angular_velocity[0]),
 ...
 */
template <huira::IsFloatingPoint T>
inline std::pair<Vec3<T>, Vec3<T>> conics(const std::array<double, 8>& elements,
                                          const huira::Time& time)
{
    SpiceDouble et = time.et();
    SpiceDouble state[6];
