...
template <std::size_t N, auto... Args>
SpectralBins<N, Args...>
SpectralBins<N, Args...>::integrate_over_data(const std::vector<double>& wavelengths,
...
template <std::size_t N, auto... Args>
float SpectralBins<N, Args...>::integrate_over_band(double min_wavelength,
...
// ================================== //
template <std::size_t N, auto... Args>
constexpr SpectralBins<N, Args...> operator+(const SpectralBins<N, Args...>& lhs,
                                             const SpectralBins<N, Args...>& rhs)
{
...
constexpr std::array<Bin, N> SpectralBins<N, Args...>::initialize_bins_static_()
{
    ...
    if constexpr (num_args == 2) {
        ...
        static_assert(
            num_args == 2 || num_args == 2 * N || num_args == N + 1,
    ...
    for (std::size_t i = 0; i < N; ++i) {
        if (bins[i].min_wavelength >= bins[i].max_wavelength) {
            throw "SpectralBins::initialize_bins_static_: Bin has min_wavelength >= max_wavelength "
                  "(inverted or zero-width bin)";
        }
    }
    ...
    if constexpr (std::same_as<TSpectral, RGB>) {
        ...
        if constexpr (has_significant_overlap) {
            ...
            for (std::size_t i = 0; i < TSpectral::size(); ++i) {
                ...
                spectral[i] =
                    rgb.integrate_over_band(bin.min_wavelength, bin.max_wavelength) / denom;
            }
            return spectral;
        } else {
