...
TSpectral black_body(double temperature, std::size_t steps)
{
    ...
    for (std::size_t i = 0; i < bins.size(); ++i) {
        ...
        std::vector<double> lambda =
            linspace(bins[i].min_wavelength, bins[i].max_wavelength, steps);
...
// Spectral irradiance calibration in the infrared. I - Ground-based and IRAS broadband
// calibrations. The Astronomical Journal, 104, 1650.
...
TSpectral visual_magnitude_to_irradiance(double visual_magnitude, TSpectral albedo)
{
    ...
    for (std::size_t i = 1; i < TSpectral::size(); ++i) {
        if (bins[i].min_wavelength < lambda_min) {
            lambda_min = bins[i].min_wavelength;
        }
        if (bins[i].max_wavelength > lambda_max) {
            lambda_max = bins[i].max_wavelength;
        }
    }
