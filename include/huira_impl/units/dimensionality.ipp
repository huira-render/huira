...
template <int L, int M, int T, int I, int O, int N, int J, int A, int S>
constexpr std::string Dimensionality<L, M, T, I, O, N, J, A, S>::get_si_unit_string(
    int power_prefix, int val, std::string unit)
{
...
constexpr std::string Dimensionality<L, M, T, I, O, N, J, A, S>::to_si_string()
{
    ...
    if (sameAs<Capacitance>()) {
        return "F";
    }
    if (sameAs<Resistance>()) {
        return "Ohm";
    }
    if (sameAs<LuminousFlux>()) {
        return "lm";
    }
    if (sameAs<Illuminance>()) {
        return "lm / m^2";
    }
    if (sameAs<Radiance>()) {
        return "W / m^2 sr";
    }
    if (sameAs<Irradiance>()) {
        return "W / m^2";
    }
    if (sameAs<RadiantIntensity>()) {
        return "W / sr";
    }
    if (sameAs<Luminance>()) {
        return "cd / m^2";
