...
namespace huira {
    ...
    static void render_point(CameraModel<TSpectral>* camera, const Vec3<float>& point, TSpectral irradiance, FrameBuffer<TSpectral>& frame_buffer) {
        ...
        Pixel star_p = camera->project_point(point);
        ...
        if (frame_buffer.has_received_power()) {
            ...
            if (unobstructed) {
                ...
                float projected_area = camera->get_projected_aperture_area(point);
        ...
        FrameBuffer<TSpectral>& frame_buffer)
    {
        ...
        for (const auto& star : scene_view.stars_) {
            Vec3<float> point = star.get_direction();
            TSpectral irradiance = star.get_irradiance();

            render_point(camera.get(), point, irradiance, frame_buffer);
        }

        // Loop over all unresolved objects:
        for (const auto& instance : scene_view.unresolved_objects_) {
            Vec3<float> point = instance.transform.position;
            TSpectral irradiance = instance.unresolved_object->get_irradiance();
