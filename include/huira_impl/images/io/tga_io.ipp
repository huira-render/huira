...
inline TGAData read_tga_raw_(const unsigned char* data, std::size_t size)
{
    ...
    if (size < 18) {
        HUIRA_THROW_ERROR("read_tga_raw_ - Data too small to be a valid TGA (" +
    ...
    std::size_t index_bytes =
        static_cast<std::size_t>(is_color_mapped ? (pixel_depth / 8) : file_channels);
    ...
    if (is_rle) {
        ...
        while (pixels_decoded < num_pixels) {
            ...
            if (packet_header & 0x80) {
                ...
                std::memcpy(
                    &pixel_indices[pixels_decoded * index_bytes], data + pos, bytes_to_read);
                pos += bytes_to_read;
        ...
        for (int x = 0; x < width; ++x) {
            ...
            if (is_color_mapped) {
                ...
                const unsigned char* src_ptr =
                    &color_map[static_cast<std::size_t>(index) *
                               static_cast<std::size_t>(cm_bytes_per_entry)];
...
inline ImageBundle<RGB> read_image_tga(const unsigned char* data, std::size_t size, bool read_alpha)
{
    ...
    for (int y = 0; y < tga_data.height; ++y) {
        for (int x = 0; x < tga_data.width; ++x) {
            std::size_t idx =
                (static_cast<std::size_t>(y) * static_cast<std::size_t>(tga_data.width) +
                 static_cast<std::size_t>(x)) *
                static_cast<std::size_t>(tga_data.channels);
