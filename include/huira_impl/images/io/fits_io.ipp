                             ...
                             const FitsMetadata& metadata)
{
    ...
    // --- Write DATAMIN / DATAMAX (actual values in the file) ---
    {
        if (!meta_copy.datamin) {
            detail::write_double_key(ff.fptr, "DATAMIN", actual_min, "Minimum pixel value", status);
        }
        if (!meta_copy.datamax) {
            detail::write_double_key(ff.fptr, "DATAMAX", actual_max, "Maximum pixel value", status);
        }
