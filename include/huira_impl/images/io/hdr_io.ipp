...
inline HDRData read_hdr_raw_(const unsigned char* data, std::size_t size)
{
    ...
    if (width <= 0 || height <= 0) {
        HUIRA_THROW_ERROR("read_hdr_raw_ - Invalid dimensions (" + std::to_string(width) + " x " +
                ...
                while (pos < width) {
                    ...
                    if (byte > 128) {
                        ...
                        for (int i = 0; i < count && pos < width; ++i) {
                            scanline[static_cast<std::size_t>(pos) * 4 +
...
inline ImageBundle<RGB> read_image_hdr(const unsigned char* data, std::size_t size)
{
    ...
    for (int y = 0; y < hdr_data.height; ++y) {
        for (int x = 0; x < hdr_data.width; ++x) {
            std::size_t idx =
                (static_cast<std::size_t>(y) * static_cast<std::size_t>(hdr_data.width) +
                 static_cast<std::size_t>(x)) *
                3;

            bundle.image(x, y) =
                RGB{hdr_data.raw_data[idx], hdr_data.raw_data[idx + 1], hdr_data.raw_data[idx + 2]};
        }
    }
...
inline ImageBundle<float> read_image_hdr_mono(const unsigned char* data, std::size_t size)
{
    ...
    for (int y = 0; y < hdr_data.height; ++y) {
        for (int x = 0; x < hdr_data.width; ++x) {
            ...
            bundle.image(x, y) =
                (hdr_data.raw_data[idx] + hdr_data.raw_data[idx + 1] + hdr_data.raw_data[idx + 2]) /
                3.0f;
        }
    }
