...
Image<PixelT>::Image() : data_{}, resolution_{0, 0}
{
}
...
Image<PixelT>::Image(Resolution resolution)
    : data_(static_cast<std::size_t>(resolution.x * resolution.y)), resolution_{resolution}
{
}
...
Image<PixelT>::Image(int width, int height)
    : data_(static_cast<std::size_t>(width * height)), resolution_{width, height}
{
}
...
Image<PixelT>::Image(int width, int height, const PixelT& fill_value)
    : data_(static_cast<std::size_t>(width * height), fill_value), resolution_{width, height}
{
}
...
PixelT Image<PixelT>::sample_bilinear(float u, float v) const
{
    ...
    if constexpr (std::is_arithmetic_v<PixelT>) {
        float result = static_cast<float>(p00) * (1.0f - fx) * (1.0f - fy) +
        ...
        return PixelT{static_cast<S>(p00.x * (1.0f - fx) * (1.0f - fy) + p10.x * fx * (1.0f - fy) +
                                     p01.x * (1.0f - fx) * fy + p11.x * fx * fy),
                      static_cast<S>(p00.y * (1.0f - fx) * (1.0f - fy) + p10.y * fx * (1.0f - fy) +
                                     p01.y * (1.0f - fx) * fy + p11.y * fx * fy),
                      static_cast<S>(p00.z * (1.0f - fx) * (1.0f - fy) + p10.z * fx * (1.0f - fy) +
                                     p01.z * (1.0f - fx) * fy + p11.z * fx * fy)};
    } else if constexpr (is_spectral_bins_v<PixelT>) {
        return p00 * ((1.0f - fx) * (1.0f - fy)) + p10 * (fx * (1.0f - fy)) +
               p01 * ((1.0f - fx) * fy) + p11 * (fx * fy);
    } else {
        // Fallback to nearest
