...
BSDFRequirements LommelSeeligerBSDF<TSpectral>::requirements() const
{
...
template <IsSpectral TSpectral>
TSpectral LommelSeeligerBSDF<TSpectral>::eval(const Vec3<float>& wo,
                                              const Vec3<float>& wi,
                                              ...
                                              const ShadingParams<TSpectral>& params) const
{
    ...
    return params.albedo / (4.0f * PI<float>() * (cos_i + cos_o));
}

template <IsSpectral TSpectral>
BSDFSample<TSpectral> LommelSeeligerBSDF<TSpectral>::sample(const Vec3<float>& wo,
                                                            const Interaction<TSpectral>& isect,
                                                            ...
                                                            float u2) const
{
    ...
    BSDFSample<TSpectral> result{};
    result.wi = wi;
    result.value = params.albedo / (4.0f * (cos_i + cos_o));
    result.pdf = hs.pdf;
    return result;
}

template <IsSpectral TSpectral>
float LommelSeeligerBSDF<TSpectral>::pdf(const Vec3<float>& wo,
                                         const Vec3<float>& wi,
                                         const Interaction<TSpectral>& isect,
                                         const ShadingParams<TSpectral>& params) const
{
