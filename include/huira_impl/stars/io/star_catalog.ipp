...
{
    star_data_.erase(std::remove_if(star_data_.begin(),
                                    star_data_.end(),
                                    [maximum_magnitude](const StarData& s) {
                                        return s.visual_magnitude > maximum_magnitude ||
                                               std::isnan(s.visual_magnitude);
                                    }),
                     star_data_.end());
}
...
inline void StarCatalog::write_star_data(const fs::path& filepath)
{
    ...
    std::copy_if(star_data_.begin(),
                 star_data_.end(),
                 std::back_inserter(valid_stars),
    ...
    HUIRA_LOG_INFO("StarCatalog - " + std::to_string(valid_stars.size()) +
                   " stars written to: " + filepath.string());
}
...
StarCatalog StarCatalog::read_star_data(const fs::path& filepath, float maximum_magnitude)
{
    ...
    if (header.magic[0] != 'H' || header.magic[1] != 'R' || header.magic[2] != 'S' ||
        header.magic[3] != 'C') {
    ...
    if (header.version != 1) {
        ...
        HUIRA_THROW_ERROR("StarCatalog - " + name + " is out of date (version = " +
                          std::to_string(static_cast<unsigned int>(header.version)) +
                          ").  Please re-generate.");
    }
    ...
    HUIRA_LOG_INFO("StarCatalog - " + std::to_string(stars.size()) +
                   " stars read from: " + filepath.string());
...
inline std::string StarCatalog::get_star_id(std::size_t index) const
{
    if (index >= star_data_.size()) {
        HUIRA_THROW_ERROR(
            "StarCatalog::get_star_id - Index out of bounds: " + std::to_string(index) + " (has " +
            std::to_string(star_data_.size()) + " stars)");
    }
