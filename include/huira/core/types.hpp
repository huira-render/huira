...
struct BasePixel {
    ...
    [[nodiscard]] constexpr const TFloat& operator[](std::size_t i) const noexcept
    {
        return i == 0 ? x : y;
    }

    constexpr BasePixel operator+(const BasePixel& rhs) const noexcept
    {
...
struct Resolution {
    Resolution(int w, int h) : width(w), height(h), x(width), y(height) {}
    ...
    {
        return Vec2<TFloat>(v.x * static_cast<TFloat>(width), v.y * static_cast<TFloat>(height));
    }

    template <IsFloatingPoint TFloat>
    Vec2<TFloat> operator*(TFloat s) const
    {
        return Vec2<TFloat>(static_cast<TFloat>(width) * s, static_cast<TFloat>(height) * s);
    }

    template <IsFloatingPoint TFloat>
    Vec2<float> operator/(const Vec2<TFloat>& v) const
    {
        return Vec2<TFloat>(static_cast<TFloat>(width) / v.x, static_cast<TFloat>(height) / v.y);
    }
...
{
    return Vec2<TFloat>(v.x * static_cast<TFloat>(res.width),
...
{
    return Vec2<TFloat>(static_cast<TFloat>(res.width) * s, static_cast<TFloat>(res.height) * s);
}
