...
struct Bin {
    ...
    constexpr Bin(double new_min, double new_max)
        : min_wavelength{new_min}, max_wavelength{new_max},
...
class SpectralBins {
  public:
    ...
    // Factory methods
    static SpectralBins integrate_over_data(const std::vector<double>& wavelengths,
    ...
    float
    integrate_over_band(double min_wavelength,
                        double max_wavelength) const; ///< Integral over specified wavelength range.
    bool valid()
        const; ///< Checks if all spectral values are valid (non-negative, not NaN, not infinite).
    bool valid_ratio() const; ///< Checks if all spectral values are valid ratios (between 0 and 1,
    ...
    static constexpr Bin get_bin(std::size_t index)
    {
        return SpectralBins::initialize_bins_static_()[index];
    }
    static constexpr std::array<Bin, N> get_all_bins()
    {
        return SpectralBins::initialize_bins_static_();
    }
