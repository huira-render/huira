...
class BSDF : public SceneObject<BSDF<TSpectral>> {
  public:
     ...
     */
    [[nodiscard]] virtual TSpectral eval(const Vec3<float>& wo,
                                         const Vec3<float>& wi,
     ...
     */
    [[nodiscard]] virtual BSDFSample<TSpectral> sample(const Vec3<float>& wo,
                                                       const Interaction<TSpectral>& isect,
     ...
     */
    [[nodiscard]] virtual float pdf(const Vec3<float>& wo,
                                    const Vec3<float>& wi,
                                    const Interaction<TSpectral>& isect,
                                    const ShadingParams<TSpectral>& params) const = 0;
