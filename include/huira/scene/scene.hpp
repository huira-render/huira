...
class Scene {
  ...
  public:
    ...
    new_unresolved_object(const units::SpectralWattsPerMeterSquared<TSpectral>& spectral_irradiance,
                          std::string name = "");
    UnresolvedObjectHandle<TSpectral>
    new_unresolved_object(const units::WattsPerMeterSquared& irradiance, std::string name = "");
    UnresolvedObjectHandle<TSpectral> new_unresolved_object_from_magnitude(double visual_magnitude,
                                                                           std::string name = "");
    UnresolvedObjectHandle<TSpectral> new_unresolved_object_from_magnitude(double visual_magnitude,
                                                                           TSpectral albedo,
                                                                           std::string name = "");
    UnresolvedObjectHandle<TSpectral>
    new_unresolved_emitter(const units::SpectralWatts<TSpectral>& spectral_power,
                           std::string name = "");
    ...
    UnresolvedObjectHandle<TSpectral> new_unresolved_sphere(units::Meter radius,
                                                            ...
                                                            float albedo,
                                                            std::string name = "");
    UnresolvedObjectHandle<TSpectral> new_unresolved_asteroid(double H,
                                                              double G,
                                                              InstanceHandle<TSpectral> sun,
                                                              std::string name = "");
    UnresolvedObjectHandle<TSpectral> new_unresolved_asteroid(
    ...
    UnresolvedObjectHandle<TSpectral>
    add_unresolved_object(std::shared_ptr<UnresolvedObject<TSpectral>> unresolved_object,
                          std::string name = "");
    ...
    void
    delete_unresolved_object(const UnresolvedObjectHandle<TSpectral>& unresolved_object_handle);
    ...
    new_bsdf_hapke(float h, float B0, float b, float c, std::string name = "");
    ...
    BSDFHandle<TSpectral> add_bsdf(std::shared_ptr<BSDF<TSpectral>> bsdf, std::string name = "");
    ...
    new_constant_density_field(TSpectral absorption, TSpectral scattering, std::string name = "");
    DensityFieldHandle<TSpectral>
    add_density_field(std::shared_ptr<DensityField<TSpectral>> density_field,
                      std::string name = "");

    PhaseFunctionHandle<TSpectral> new_isotropic_phase_function(std::string name = "");
    PhaseFunctionHandle<TSpectral>
    add_phase_function(std::shared_ptr<PhaseFunction<TSpectral>> phase_function,
                       std::string name = "");

    MediumHandle<TSpectral> new_medium(DensityFieldHandle<TSpectral> density_field_handle,
    ...
    TextureHandle<Vec3<float>> add_normal_texture(Image<Vec3<float>>&& image,
                                                  std::string name = "");
    ...
    void
    load_stars(const fs::path& star_catalog_path, const Time& time, float min_magnitude = 100.f);

    void load_dynamic_stars(const fs::path& star_catalog_path,
                            const Time& time,
                            float min_magnitude = 100.f);
  ...
  private:
    ...
    find_node_in_tree_(const std::shared_ptr<Node<TSpectral>>& current,
