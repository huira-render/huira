...
struct PositionCallback {
    ...
    virtual void evaluate(const Time& epoch,
                          const Rotation<double>& rotation,
                          const Vec3<double>& angular_rate) = 0;
...
struct RotationCallback {
    ...
    virtual void
    evaluate(const Time& epoch, const Vec3<double>& position, const Vec3<double>& velocity) = 0;
