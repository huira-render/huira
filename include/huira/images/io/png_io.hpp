...
namespace huira {
    ...
    ImageBundle<RGB> read_image_png(const unsigned char* data, std::size_t size, bool read_alpha = true);

    ImageBundle<float> read_image_png_mono(const fs::path& filepath, bool read_alpha = true);
    ImageBundle<float> read_image_png_mono(const unsigned char* data, std::size_t size, bool read_alpha = true);
