...
template <typename T>
concept IsUnsignedInteger = std::same_as<T, std::uint8_t> || std::same_as<T, std::uint16_t> ||
                            std::same_as<T, std::uint32_t> || std::same_as<T, std::uint64_t> ||
                            std::same_as<T, std::size_t>;

// @concept IsSignedInteger
template <typename T>
concept IsSignedInteger = std::same_as<T, std::int8_t> || std::same_as<T, std::int16_t> ||
                          std::same_as<T, std::int32_t> || std::same_as<T, std::int64_t>;

/// @concept IsInteger
template <typename T>
concept IsInteger = IsUnsignedInteger<T> || IsSignedInteger<T>;

/// @concept IsNumeric
template <typename T>
concept IsNumeric = IsFloatingPoint<T> || IsInteger<T>;
