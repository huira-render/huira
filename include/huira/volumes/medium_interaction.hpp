...
struct MediumInteraction {
    ...
    MediumInteraction(const Vec3<float>& p_set,
                      float t_set,
                      const Vec3<float>& wo_set,
                      const MediumProperties<TSpectral>& properties_set,
                      const PhaseFunction<TSpectral>* phase_function_set)
        : p(p_set), t(t_set), wo(wo_set), properties(properties_set),
          phase_function(phase_function_set)
    {
    }

    [[nodiscard]] bool is_valid() const { return phase_function != nullptr; }
};
