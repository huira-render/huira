...
class Light : public SceneObject<Light<TSpectral>> {
  public:
    ...
    virtual std::optional<LightSample<TSpectral>> sample_li(const Interaction<TSpectral>& ref,
                                                            const Transform<float>& light_to_world,
                                                            Sampler<float>& sampler) const = 0;

    virtual float pdf_li(const Interaction<TSpectral>& ref,
                         const Transform<float>& light_to_world,
                         const Vec3<float>& wi) const = 0;

    virtual TSpectral radiance(const Vec3<float>& point_on_light,
                               const Vec3<float>& outgoing_direction) const = 0;

    virtual TSpectral irradiance_at(const Vec3<float>& position,
                                    const Transform<float>& light_to_world) const = 0;
