...
{
    return (static_cast<std::uint64_t>(tyc1) << 20) | (static_cast<std::uint64_t>(tyc2) << 4) |
           static_cast<std::uint64_t>(tyc3);
}

inline void
unpack_tycho2_id(std::uint64_t id, std::uint16_t& tyc1, std::uint16_t& tyc2, std::uint8_t& tyc3)
{
...
inline std::string format_tycho2_id(std::uint64_t id)
{
    ...
    return "TYC " + std::to_string(static_cast<unsigned int>(tyc1)) + "-" +
