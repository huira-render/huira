...
inline void bind_spice(py::module_& m)
{
    ...
    spice_mod.def(
        "furnsh", &huira::spice::furnsh, py::arg("file_path"), "Load a SPICE kernel file");

    spice_mod.def("furnsh_relative_to_file",
                  &huira::spice::furnsh_relative_to_file,
                  py::arg("kernel_path"),
                  "Load a SPICE kernel using a path relative to the calling file");

    spice_mod.def("load_default_pck",
                  &huira::spice::load_default_pck,
