...
namespace huira {
    ...
    void bind_scene_view(py::module_& m) {
        ...
        py::class_<SV>(m, "SceneView")
            ...
            .def("resolve_hit",
                static_cast<Interaction<TSpectral>(SV::*)(const Ray<TSpectral>&, const HitRecord&) const>(&SV::resolve_hit),
                py::arg("ray"), py::arg("hit"),
                "Resolve a HitRecord into a full Interaction")

            .def("intersect_batch",
                static_cast<std::vector<HitRecord>(SV::*)(const std::vector<Ray<TSpectral>>&, float) const>(&SV::intersect),
                py::arg("rays"), py::arg("time") = 0.5f,
                "Trace a batch of rays against the scene and return HitRecords")
            .def("occluded_batch",
                static_cast<std::vector<bool>(SV::*)(const std::vector<Ray<TSpectral>>&, float, float) const>(&SV::occluded),
                py::arg("rays"), py::arg("t_far"), py::arg("time") = 0.5f,
                "Test whether a batch of rays are occluded within distance t_far")
            .def("resolve_hits_batch",
                static_cast<std::vector<Interaction<TSpectral>>(SV::*)(const std::vector<Ray<TSpectral>>&, const std::vector<HitRecord>&) const>(&SV::resolve_hits),
                py::arg("rays"), py::arg("hits"),
