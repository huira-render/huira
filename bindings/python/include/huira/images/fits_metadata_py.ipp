...
inline void bind_fits_metadata(py::module_& m)
{
    ...
    py::class_<FitsKeyword>(m, "FitsKeyword")
        .def(py::init<>())
        .def(py::init([](const std::string& key, py::object value, const std::string& comment) {
                 FitsKeyword kw;
                 kw.key = key;
                 kw.comment = comment;
        ...
        .def_property(
            ...
            [](const FitsKeyword& kw) -> py::object {
                return std::visit([](auto&& v) -> py::object { return py::cast(v); }, kw.value);
            },
    ...
    py::class_<FitsMetadata>(m, "FitsMetadata")
        ...
        .def("__repr__", [](const FitsMetadata& meta) {
            std::ostringstream os;
            os << "FitsMetadata(";
            if (!meta.object.empty()) {
                os << "object='" << meta.object << "', ";
            }
            if (!meta.telescop.empty()) {
                os << "telescop='" << meta.telescop << "', ";
            }
            if (meta.has_wcs()) {
                os << "wcs=True, ";
            }
