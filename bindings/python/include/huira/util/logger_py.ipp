...
namespace huira {
    ...
    inline void bind_logger(py::module_& m) {
        ...
        m.def("log", [](LogLevel level, const std::string& message) {
            // Route Python strings directly into the C++ singleton
            Logger::log(level, message);
            }, py::arg("level"), py::arg("message"), "Log a message from Python to the Huira logger.");
    }
}
