...
inline void bind_hit_record(py::module_& m)
{
    ...
    py::class_<HitRecord>(m, "HitRecord", "Result of a ray–scene intersection query")
        .def(py::init<>())
        ...
        .def("hit", &HitRecord::hit, "Returns true if the record represents a valid intersection")
        ...
        .def("__repr__", [](const HitRecord& h) {
            ...
            if (!h.hit()) {
                ...
                ss << "HitRecord(t=" << h.t << ", inst_id=" << h.inst_id
                   << ", geom_id=" << h.geom_id << ", prim_id=" << h.prim_id << ")";
            }
            return ss.str();
        });
}
