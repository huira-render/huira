...
inline void bind_rotation(py::module_& m)
{
    ...
    py::class_<Rot>(m,
        ...
        .def_static(
            "from_local_to_parent_axis_angle",
        ...
        .def_static(
            "from_parent_to_local_axis_angle",
            [](py::array_t<double> axis_arr, const py::object& angle) {
                auto buf = axis_arr.request();
                if (buf.ndim != 1 || buf.shape[0] != 3) {
                    throw std::runtime_error("Axis must be a 1-D array of length 3");
        ...
        // ==============================================================
        .def_static(
            "extrinsic_euler_angles",
            [](const py::object& a1,
               const py::object& a2,
               const py::object& a3,
               const std::string& seq) {
                return Rot::extrinsic_euler_angles(detail::unit_from_py<units::Radian>(a1),
        ...
        .def_static(
            "intrinsic_euler_angles",
            [](const py::object& a1,
               const py::object& a2,
               const py::object& a3,
               const std::string& seq) {
                return Rot::intrinsic_euler_angles(detail::unit_from_py<units::Radian>(a1),
                                                   detail::unit_from_py<units::Radian>(a2),
                                                   detail::unit_from_py<units::Radian>(a3),
                                                   seq);
            },
        ...
        .def_static(
            "from_basis_vectors",
            [](py::array_t<double> x_arr, py::array_t<double> y_arr, py::array_t<double> z_arr) {
                ...
                return Rot::from_basis_vectors(
                    parse(x_arr, "x_axis"), parse(y_arr, "y_axis"), parse(z_arr, "z_axis"));
            },
            py::arg("x_axis"),
            py::arg("y_axis"),
            py::arg("z_axis"),
        ...
        // ==============================================================
        .def(
            "__mul__", [](const Rot& a, const Rot& b) { return a * b; }, py::is_operator())
        .def(
            "__imul__",
            [](Rot& a, const Rot& b) {
                a *= b;
            ...
            py::is_operator())
        .def(
            "__mul__", [](const Rot& r, const Vec3<double>& v) { return r * v; }, py::is_operator())

        // Apply rotation to a numpy vector:
        .def(
            "apply",
            [](const Rot& r, py::array_t<double> arr) {
                auto buf = arr.request();
                if (buf.ndim != 1 || buf.shape[0] != 3) {
